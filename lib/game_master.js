  ...
  if (!user.profile_image_url) {
    ...
    request({ url: user.profile_image_url, encoding: null }, function(err, res, body) {
      var ext = mime.extension(res.headers['content-type'])
        ...
        , path_to_profiles = path.normalize(path.join(__dirname + '/../public/images/profiles/'))
        , profile = path.normalize(path.join(__dirname + '/../public/images/profiles/', filename));

      fs.mkdir(path_to_profiles, function(err) {
        fs.writeFile(profile, body, function(err) {
...
GameMaster.prototype.throwDetonator = function(socket, user_id) {
  if (this.players[socket.id] && this.players[socket.id].profile_image_url) {
...
GameMaster.prototype.thankYou = function() {
  this.io.of('/player').emit('thank');
}
